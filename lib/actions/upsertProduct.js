...
async function action (msg, cfg) {
    try {
        const authResponse = await axios.post(`${cfg.url}/rest/all/V1/integration/admin/token`,
            {username: cfg.username, password: cfg.password});
        console.log(JSON.stringify(msg.body))
        const service = axios.create({
            baseURL: cfg.url + '/rest/all/V1',
...
async function getAttributes(service, inputAttributes) {
    ...
    for (let inputAttr of inputAttributes) {
        if (!inputAttr.value || inputAttr.value === '') {
            continue
        }
        ...
        if (attr.frontend_input === 'select') {
            attrValue = attr.options.filter((option) => option.label.toLowerCase() === inputAttr.value.toLowerCase()).pop();
            if (attrValue === undefined) {

                let attrUpdateResponse = await service.post(`/products/attributes/${attr.attribute_code}/options`, {
                    option: {label: inputAttr.value}
                });
                attr = (await service.get(`/products/attributes/${attr.attribute_code}`)).data;
                console.log(attr.options, inputAttr)
                attrValue = attr.options.filter((option) => option.label.toLowerCase() === inputAttr.value.toLowerCase()).pop();
            }
            if (!attrValue) {
                console.log(`problems with attribute`)
