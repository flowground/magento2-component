 ...
 * categories notation:
 * {
 *   categories: 'Root->Root sub category->...->Category name;Root->Root sub category->...->Category name'
 * }
 */
...
async function action (msg, cfg) {
    try {
        ...
        if (msg.body.categories) {
            let categoriesIds = await getCategoriesIds(service, msg.body.categories.split(';'));
...
async function getAttributes(service, inputAttributes) {
    ...
    for (let inputAttr of inputAttributes) {
        let attr = attrs.filter((attr) => inputAttr.key.toLowerCase() === attr.attribute_code.toLowerCase()).pop();
        if (!attr) {
            let attrData = {
