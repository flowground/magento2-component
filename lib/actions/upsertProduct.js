...
async function action (msg, cfg) {
    try {
        ...
        let price = Number(msg.body.price)
        let productData = {
            name: msg.body.name,
            status: price > 0 && Math.abs(Number(msg.body.qty)) > 0 ? msg.body.status : 2,
            price,
        ...
        let productResponse = await service.get(`/products/${msg.body.sku}?storeId=0`);
        if (productResponse.status === 200) {
            let updateProductResponse = await service.put(`/products/${msg.body.sku}?storeId=0`, {
                product: productData
            });
            ...
            let createProductResponse = await service.post('/products/?storeId=0', {
                product: productData
            });
...
async function uploadPicture(service, sku, name, isMain, imageUrl) {
    let imagesResponse = await service.get(`/products/${sku}/media?storeId=0`);
    ...
    if (!itemImagesHashes.includes(fileName)) {
        ...
        let createResponse = await service.post(`/products/${sku}/media?storeId=0`, imageData);
...
async function getCategoriesIds(service, inputCategories) {
    ...
    async function createNewCategory(category) {
        ...
        let createCategoryResponse = await service.post('/categories?storeId=0', {category});
...
async function getAttributes(service, inputAttributes) {
    ...
    for (let inputAttr of inputAttributes) {
        ...
        let attr = attrs.filter((attr) => {
            return inputAttr.key.toLowerCase() === attr.attribute_code.toLowerCase()
        }).pop();
        ...
        if (!attr) {
            ...
            let attrCreateResponse = await service.post('/products/attributes?storeId=0', {
                attribute: attrData
            });
        ...
        if (attr.frontend_input === 'select') {
            ...
            if (attrValue === undefined) {
                ...
                let attrUpdateResponse = await service.post(`/products/attributes/${attr.attribute_code}/options?storeId=0`, {
                    option: {label: inputAttr.value}
                });
                attr = (await service.get(`/products/attributes/${attr.attribute_code}`)).data;
                attrValue = attr.options.filter((option) => option.label === inputAttr.value).pop();
            }
        ...
        if (attr.frontend_input === 'multiselect') {
            ...
            await Promise.all(values.map(async (v) => {
                ...
                if (magValue === undefined) {
                    ...
                    let attrUpdateResponse = await service.put(`/products/attributes/${attr.attribute_code}?storeId=0`, {
                        attribute: attr
                    });
                    attr = attrUpdateResponse.data;
                    magValue = attr.options.filter((option) => option.label === v).pop();
                }
                if (!attrValue.includes(magValue.value)) {
...
async function getAttributeSetId(service, inputAttributeSetName) {
    ...
    if (set) {
        ...
        let createSetResponse = await service.post('/products/attribute-sets?storeId=0', {
            attributeSet: {
