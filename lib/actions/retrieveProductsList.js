...
async function exec(msg, cfg) {
    ...
    const productResponse = await service.get(`${baseUrl}/products?${queryString}`);
...
exports.getMetaModel = async function getMetaModel(cfg) {
    return await getMetaModelHelper(cfg, './lib/schemas/retrieveProductsList.in.json',
        './lib/schemas/retrieveProductsList.out.json');
};
