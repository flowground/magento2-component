...
async function action (msg, cfg) {
    try {
        ...
        await Promise.all(msg.body.pieces.map(async (p) => {
             ...
             */
            let price = Math.round(mainProduct.price * cfg.priceMultiplier * 100)/100;
            ...
            productData.tier_prices = mainProduct.tier_prices.map((price) => {
                price.value = Math.round(price.value * cfg.priceMultiplier * 100)/100
                return price
            });
            ...
            if (productResponse.status !== 200) {
                ...
                let updateProductResponse = await service.put(`/products/${p.sku}`, {
                    product: productData
                });
