...
async function action(msg, cfg, snapshot) {
    try {
        ...
        let products = (await axios.get(`${API_URL}/rest/V1/products?searchCriteria=searchCriteria`, options)).data.items;
        let result = [];
        for (let product of products) {
            debug("product: %o", product.sku);
            ...
            debug("fullProduct: %o",fullProduct);
            ...
            debug("emitting data %o", result);
            this.emit('data', messages.newMessageWithBody(result));
        }
        debug("emitting snapshot...");
